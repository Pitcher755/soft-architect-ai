...
Para la concepción y documentoación de este proyecto, se ha utilizado una metodología híbrida humano-IA, actuando la IA (Modelos LLM avanzados) como **Arquitecto Consultor**.

### Herramientas utilizadas:
* **Copilot & LLMs:** Utilizados no solo para autocompletar código, sino para:
    * Generar estructuras de documentoación (como este archivo).
    * Simular roles (Business Analyst, DevOps Engineer) para validar ideas.
    * Sintetizar grandes volúmenes de documentoación técnica.
...
3.  **Estrategia de Despliegue (CI/CD):**
    * Se diseñan pipelines (GitHub Actions) que no solo corren pruebas, sino que verifican la integridad de la base de conocimiento RAG.
...
5.  **Automatización de Documentoación (Docs-as-Code):**
    * Se ha implementado un pipeline de **Integración Continua de Conocimiento**.
    * La documentoación reside junto al código (Markdown en Git), pero se despliega automáticamente a Notion mediante **n8n** y Webhooks.
    * *Justificación:* Elimina la desincronización entre lo que hace el código y lo que dice la documentoación, aplicando principios DevOps a la gestión del conocimiento.
...
## 1. Introducción
Este documentoo justifica las decisiones técnicas tomadas para el desarrollo de **SoftArchitect AI**, vinculándolas directamente con los conocimientos adquiridos en el Máster de Desarrollo con IA. El objetivo no es solo construir una herramienta útil, sino demostrar la capacidad de diseñar sistemas complejos, escalables y desplegables.
...
## 4. Aplicación del Módulo de Calidad y Pruebaing
* **Frontend:** Pruebas de Widgets en Flutter para asegurar que los formularios de requisitos y la visualización de respuestas son robustos.
* **Backend:** Pruebas unitarios en Python (Pyprueba) para validar la lógica de construcción de prompts y la conexión con ChromaDB.
* **RAG Evaluation:** (Fase futura) Implementación de "RAGAS" para medir la precisión y fidelidad de las respuestas generadas por el sistema frente a la documentoación base.
...
* **Fase de Desarrollo:** Se utiliza `flutter ejecutar -d web-server` en el servidor. VS Code realiza un *Port Forwarding* automático a través del túnel SSH, permitiendo ver y depurar la aplicación en el navegador del portátil local (`localhost:8080`) como si se ejecutara nativamente.
...
* **Objetivo:** Validar si el *Master Workflow* es capaz de generar entregables útiles (Vision Statements, ADRs, Dockerarchivos) sin intervención humana creativa.
* **Resultadoado:** La simulación confirmó que, con el contexto adecuado, el sistema puede actuar como un "Arquitecto Senior", reduciendo la incertidumbre del proyecto antes de la fase de desarrollo.

## 8. Evolución del RAG: Arquitectura de Contexto Estructurado
Se ha pivotado de un enfoque RAG tradicional (búsqueda semántica en documentoos desestructurados) a una **Arquitectura de Contexto Estructurado**.
...
Durante la fase de validación técnica ("The Fire Prueba"), se identificó un cuello de botella crítico en servidores domésticos antiguos (ausencia de instrucciones AVX en CPUs pre-2011), haciendo inviable la ejecución local de modelos, incluso los más ligeros ("TinyLlama").

### Decisión de Diseño:
En lugar de restringir el software a hardware de gama alta, se optó por una **Arquitectura Híbrida**. Se integra **Groq Cloud** como fallback transparente. Esto permite desarrollar y pruebaear la lógica del RAG en el HomeLab (usando nube rápida) y desplegar en producción local o en el portátil potente (usando Ollama) con el mismo codebase. Es un ejemplo práctico de cómo la infraestructura dicta decisiones de arquitectura de software.
...
Se integró el `nvidia-container-toolkit` en el sistema operativo base (Linux) y se configuró el `daemon.json` de Docker para permitir el ejecutartime de NVIDIA.
