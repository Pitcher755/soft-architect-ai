...
* **Your HomeLab:** Has the repository cloned, Docker ejecutarning, AI models downloaded, and the development environment.
* **The Tunnel:** VS Code connects via SSH and shows you the server's archivos as if they were on your hard drive.
...
I assume your HomeLab ejecutars Linux (Ubuntu/Debian/Zorin).

* Make sure you have SSH access from your laptop.
* Install **Docker** and **Git** on the HomeLab.
* Crear the proyecto carpeta: `mkdir ~/proyectos/softarchitect`.
...
1. In VS Code, click "Open Carpeta".
2. Surprise! You're not browsing your laptop, you're browsing the HomeLab's archivos.
3. Open the integrated terminal in VS Code (`Ctrl + ñ`). **That terminal is your HomeLab's terminal**.

Now, ejecutar in that integrated terminal:
...
**Resultado:** Everything (Ollama, ChromaDB, Backend) is ejecutarning on the server. Your laptop isn't consuming RAM or storing archivos.
...
1. In the remote terminal (in VS Code), ejecutar:
```bash
...
* *Disadvantage:* You're pruebaing the Web version, not the native desktop version, but for functional development it's identical.
...
### ⚙️ Critical Environment Configuracións (HomeLab)

For the experience to be smooth as silk, ejecutar these adjustments once on your server (SSH terminal):

#### 1. Docker without `sudo` (Vital for VS Code)

VS Code tries to ejecutar Docker commands with your user. If it needs `sudo` every time, it will fail silently.
...
When you connect via SSH, you'll see that your local extensions appear grayed out or with a botón "Install in SSH: HomeLab".
...
#### 4. Increase Archivo Watchers

Flutter and VS Code watch thousands of archivos. The default limit on Linux is low and can cause strange errors.
...
* **Magic IPs:** Tailscale assigns a fixed IP (e.g., `100.x.y.z`) to your HomeLab. ALWAYS use this IP in VS Code Remote-SSH, even when you're at home. This way you don't have to change the configuración if you go to a coffee shop.
...
### 7.1. Docker Compose Configuración
Minimum configuración to validate hardware on the HomeLab. This service exposes the Ollama API on port 11434.
...
### 7.2. Hardware Validation ("The Fire Prueba")
...
2. **Download the prueba model (Phi-3.5 Mini):**
...
3. **Inference Prueba (Smoke Prueba):**
Ejecutar a simple prompt to verify latency and operation.
```bash
...
## 8. Intelligence Engine Configuración (Provider Selector)

SoftArchitect AI can work in two modes. Choose the one that fits your hardware by editing the `.env` archivo.

### Option A: Local Mode (Total Privacy) 🔒
*Recommended for:* Modern laptops (M1/M2/M3, Ryzen 5000+, Intel 11th+) or PCs with NVIDIA GPU.
*Requirement:* Have Ollama ejecutarning (`docker compose up`).
