...
This documento describes the automation pipelines that keep the proyecto's documentoation and code synchronized.

## 1. Documentoation Synchronization Pipeline (Docs Sync)
...
1.  **Trigger:** `GitHub Webhook` detects a `push` that modifies Markdown archivos (`.md`).
2.  **Extraction:** Downloads the "raw" content of the modified archivo.
...
4.  **Decision:**
    ...
    * **If not exists:** Crears a new page in the Database, assigning the title and the `Local Path` property.

### Required Configuración
* **Notion Integración Token:** Internal token with read/write permissions.
...
## 2. Hybrid Development Infraestructura
