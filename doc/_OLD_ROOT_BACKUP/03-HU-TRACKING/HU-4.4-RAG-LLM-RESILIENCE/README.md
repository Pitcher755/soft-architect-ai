...
### 5. Backend: Chat History Support (Phase 6) 🧠

> **Commit:** `01eec76`
> **Status:** ✅ Implemented
> **Priority:** 🟡 MEDIUM

**Objetivo:** Implementar memoria conversacional en backend para que el LLM tenga contexto de mensajes previos.

**Archivos Modificados:**
- `src/server/app/domain/schemas/chat.py` (+70 LOC)
- `src/server/app/api/dependencies.py` (+15 LOC)
- `src/server/app/services/rag/orchestrator.py` (+2 LOC)
...
class ChatRequest(BaseModel):
    ...
    history: list[dict[str, str]] = Field(
        ...
        description="Chat history (max 20 messages, 5000 chars/msg)",
    )

    @model_validator(mode="after")
    def validate_history(self) -> Self:
        """Validate history: roles, content, XSS sanitization."""
...
- ✅ Estructura: System → History → Context → Query

**Tests:** 21 tests (9 unit + 7 template + 5 integration)

**Limitaciones:**
- ❌ Max 20 mensajes (hardcoded, pequeño para proyectos grandes)
- ❌ Max 5000 chars/mensaje (hardcoded, modelo soporta 32K)
- ❌ Frontend NO envía historial (backend listo pero no usado)
- ✅ **Resuelto en Phase 7**

---

### 6. Configurable Chat Limits (Phase 7) ⚙️

> **Commit:** `3786589`
> **Status:** ✅ Implemented
> **Priority:** 🟡 HIGH

**Objetivo:** Hacer límites configurables vía environment variables e integrar frontend para enviar historial.

**Archivos Modificados:**

**Backend (3 archivos):**
- `src/server/app/core/config.py` (+10 LOC)
- `src/server/app/domain/schemas/chat.py` (+20 LOC)
- `src/server/.env.example` (+12 LOC)

**Frontend (1 archivo):**
- `src/client/lib/features/chat/data/repositories/chat_repository_impl.dart` (+40 LOC)

**Configuration (2 archivos):**
- `infrastructure/docker-compose.yml` (+4 LOC)
- `src/server/app/api/dependencies.py` (+8 LOC)

**Environment Variables (NEW):**
```bash
# Chat History Configuration
CHAT_MAX_HISTORY_MESSAGES=100  # Default (5x Phase 6)
CHAT_MAX_MESSAGE_LENGTH=20000  # Default (4x Phase 6)
```
...
class Settings(BaseSettings):
    ...
    CHAT_MAX_MESSAGE_LENGTH: int = 20000
```

**Dynamic Validation:**
```python
# Validators read from settings (not hardcoded)
...
Stream<ChatStreamEvent> sendMessageStream(...) async* {
  ...
  // 5. Graceful degradation (continues if SQLite fails)
}
```
...
- ✅ **Configuración sin downtime:** Editar `.env` y restart (no recompilación)
- ✅ **Frontend integrado:** Carga y envía últimos 100 mensajes automáticamente
- ✅ **Graceful degradation:** Continúa sin historial si SQLite falla
- ✅ **Proyectos grandes viables:** 25+ documentos soportados

**Tests:** 17 tests actualizados (13 unit + 4 integration)
...
docker-compose up -d  # Reads from docker-compose.yml
...
**Benefits:**
- 📈 Escalabilidad: Soporte para proyectos grandes
- ⚙️ Configurabilidad: Tuning sin conocimiento de código
- 🚀 Performance: 100% utilización del contexto del modelo
- 🧠 Memoria: Conversaciones extensas con contexto completo
