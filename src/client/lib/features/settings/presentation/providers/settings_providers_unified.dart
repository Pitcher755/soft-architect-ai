...
class AppSettings {
  ...
  factory AppSettings.fromJson(Map<String, dynamic> json) => AppSettings(
    themeMode:
        ThemeMode.values[json['themeMode'] ?? 1], // 1 = ThemeMode.dark
...
class SettingsNotifier extends Notifier<AppSettings> {
  ...
  void updateMemoryOptimization({required bool enableMemoryOptimization}) {
    state =
        state.copyWith(enableMemoryOptimization: enableMemoryOptimization);
    _saveSettings();
  }
...
final globalZoomProvider = Provider<double>((ref) {
  return ref.watch(settingsProvider.select((s) => s.globalZoom));
});

final enableZoomShortcutsProvider = Provider<bool>((ref) {
  return ref.watch(settingsProvider.select((s) => s.enableZoomShortcuts));
});

final enableAnimationsProvider = Provider<bool>((ref) {
  return ref.watch(settingsProvider.select((s) => s.enableAnimations));
});

final enableMemoryOptimizationProvider = Provider<bool>((ref) {
  return ref
      .watch(settingsProvider.select((s) => s.enableMemoryOptimization));
});

final userNameProvider = Provider<String>((ref) {
  return ref.watch(settingsProvider.select((s) => s.userName));
});

final avatarIndexProvider = Provider<int>((ref) {
  return ref.watch(settingsProvider.select((s) => s.avatarIndex));
});

final customAvatarPathProvider = Provider<String?>((ref) {
  return ref.watch(settingsProvider.select((s) => s.customAvatarPath));
});

final projectDirectoryProvider = Provider<String?>((ref) {
  return ref.watch(settingsProvider.select((s) => s.projectDirectory));
});
...
final lastProjectProvider =
    NotifierProvider<LastProjectNotifier, String?>(LastProjectNotifier.new);
